          ...
          capacites.match(/.*?\d+\)/g).map (line) ->
            ...
            comp.split(/,/).forEach (x) ->
              ...
              if not _.isEmpty(x)
                ...
                # console.log "#{JSON.stringify currentMat, null, 2}"
                if tmpComp?
                  if _.isEmpty(currentMat.connaissances[tmpComp.code])
                    currentMat.connaissances[tmpComp.code] = []
                  currentMat.connaissances[tmpComp.code].push("connaissance : #{capacite}")
                  currentMat.connaissances[tmpComp.code] = _.uniq(currentMat.connaissances[tmpComp.code])
                else
                  console.error "Competence inconnue #{x}, #{JSON.stringify currentMat, null, 2}"
        else
          capacites.split(/\./).forEach (x) ->
            if _.isEmpty(currentMat.connaissances['Inconnu'])
              currentMat.connaissances['Inconnu'] = []
            currentMat.connaissances['Inconnu'].push("connaissance : #{x}")
            currentMat.connaissances['Inconnu'] = _.uniq(currentMat.connaissances['Inconnu'])
